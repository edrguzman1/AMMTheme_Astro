...
<head>
    ...
    <link rel="icon" type="image/svg+xml" href="<?php echo get_template_directory_uri(); ?>/template/images/amm-favicon.png">
...
<body <?php body_class("bg-white text-slate-700 font-sans antialiased selection:bg-cyan-500 selection:text-white relative"); ?>>
    ...
    <header class="fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-white border-b border-slate-200" id="main-navbar">
        ...
        <script type="module">
            const r=document.getElementById("main-navbar");
            window.addEventListener("scroll",()=>{window.scrollY>20?(r?.classList.remove("bg-white"),r?.classList.add("bg-white/80","backdrop-blur-xl","shadow-sm","border-slate-200/60")):(r?.classList.add("bg-white"),r?.classList.remove("bg-white/80","backdrop-blur-xl","shadow-sm","border-slate-200/60"))});
            const c=document.getElementById("mobile-menu-btn"),a=document.getElementById("mobile-menu"),e=c?.querySelectorAll("span");
            let l=!1;
